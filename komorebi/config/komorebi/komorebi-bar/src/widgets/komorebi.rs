    ...
    fn render(&mut self, ctx: &Context, ui: &mut Ui, config: &mut RenderConfig) {
            ...
            if workspaces.enable {
                ...
                if !komorebi_notification_state.workspaces.is_empty() {
                    ...
                    config.apply_on_widget(false, ui, |ui| {
                        for (i, (ws, containers, _, should_show, is_empty)) in
                            ...
                            .show(ui, |ui| {
                                    ...
                                    || (is_selected && matches!(format, WorkspacesDisplayFormat::AllIconsAndTextOnSelected | WorkspacesDisplayFormat::Existing(DisplayFormat::IconAndTextOnSelected)))
                                {
                                     if is_selected {
                                        ...
                                        ui.add(Label::new(RichText::new(ws.to_string()).color(Color32::DARK_GRAY)).selectable(false))
                                    }
                                    else {
        ...
        if let Some(focused_container_config) = self.focused_container {
            if focused_container_config.enable {
                ...
                for (container_i, container) in komorebi_notification_state.containers.iter().enumerate() {
                ...
                if !titles.is_empty() {
                    config.apply_on_widget(false, ui, |ui| {
                        ...
                        for (i, (title, icon)) in iter.enumerate() {
                            ...
                            let text_color = if container_i == komorebi_notification_state.focused_container_idx && i == focused_window_idx { ctx.style().visuals.selection.stroke.color } else { ui.style().visuals.text_color() };
...
pub struct KomorebiNotificationState {
    ...
    pub containers: Vec<KomorebiNotificationStateContainerInformation>,
...
impl KomorebiNotificationState {
    ...
    pub fn handle_notification(
        ...
        let ws = &monitor.workspaces()[focused_workspace_idx];
        if let Some(container) = ws.monocle_container() {
            containers.push(container.into());
            self.focused_container_idx = 0;
        } else {
            self.focused_container_idx = ws.focused_container_idx();
        }
        ...
        for (i, floating_window) in ws.floating_windows().iter().enumerate() {
            containers.push(floating_window.into());

            if floating_window.is_focused() {
                self.focused_container_idx = len + i;
            }
        }

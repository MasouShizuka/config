...
pub fn handle_notifications(wm: Arc<Mutex<WindowManager>>) -> color_eyre::Result<()> {
    ...
    for notification in receiver {
        let mut wm_lock = wm.lock();
        let mouse_follows_focus = wm_lock.mouse_follows_focus;
        let window = Window::from(*notification);
        let _ = window.focus(mouse_follows_focus);

        // NOTE: 令 animation 结束后 focus 窗口对应的 monitor
        if let Some(monitor_idx) = wm_lock.monitor_idx_from_window(window) {
            wm_lock.focus_monitor(monitor_idx).unwrap_or(());
        }
    }

...
local zsh = require("config.launching_programs").zsh
local powershell = require("config.launching_programs").powershell
local arch = require("config.launching_programs").arch
...
local function get_shell_panes()
    return wezterm.GLOBAL.shell_panes or {}
end

local function set_shell_panes(shell_panes)
    wezterm.GLOBAL.shell_panes = shell_panes or {}
end
...
local function add_shell_pane(pane_id, command)
    local pane_id = tostring(pane_id)
    local command = command or {}

    local shell_panes = get_shell_panes()
    if shell_panes[pane_id] == nil then
        shell_panes[pane_id] = command
        set_shell_panes(shell_panes)
    end
end
...
return {
    ...
    leader = { key = "phys:s", mods = "CTRL|SHIFT", timeout_milliseconds = 1000 },

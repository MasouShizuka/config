...
| 文件                 | 路径     | 说明                                                                                                                                                                                                 |
| -------------------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| aria2.sh             | `aria2/` | aria2 下载器启动脚本                                                                                                                                                                                 |
| aria2.conf           | `aria2/` | aria2 设置                                                                                                                                                                                     |
...
| delete.sh            | `aria2/` | 文件删除脚本。在下载停止后执行(`on-download-stop`)，当下载错误或删除正在下载的任务后自动删除相关文件，并自动清理控制文件(*.aria2)、种子文件(*.torrent)和空目录，防止不必要的磁盘空间占用。（默认启用） |
...
- 脚本中用到了 `jq` 命令，因此需要从 [jq 官网](https://stedolan.github.io/jq/) 下载 `jq-win64.exe`，并将其移动到 `Git/usr/bin/jq.exe`

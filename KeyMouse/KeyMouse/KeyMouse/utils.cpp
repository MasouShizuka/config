...
void SetTagMapThread(HWND hWnd, bool bUseCache) {
	...
	std::queue<string> TagQueue = TC.AllocTag(int(pElementVec->size() + pWindowVec->size()));
...
/**
* @brief: a thread function for painting equidistantly on the transparent window.
*
* @param: HWND hWnd: the handle of main window.
*         bool bUseCache: whether use cache.
* @return: void
*/
void SetTagMapThreadForHint(HWND hWnd, bool bUseCache) {
	HWND *phMainWnd = reinterpret_cast<HWND *>(GetClassLongPtr(hWnd, 0));
	KeyMouse::Context *pCtx = GetContext(*phMainWnd);
	if (pCtx == nullptr)
		return;
    ...
    int Width;
    int Height;
    if (pCtx->GetProfile().show_hint_only_current_monitor) {
        RECT prc;
        GetWindowRect(GetForegroundWindow(), &prc);
    ...
    int hint_distance = pCtx->GetProfile().hint_distance;
    int num_horizontal = Width / hint_distance + 1;
    int num_vertical = Height / hint_distance + 1;

	KeyMouse::PTagMap pTagMap(new std::map<string, KeyMouse::ElementInfo>);
	KeyMouse::PTagMap pWindowMap(new std::map<string, KeyMouse::ElementInfo>);
	KeyMouse::TagCreator TC;
	std::queue<string> TagQueue = TC.AllocTag(num_horizontal * num_vertical);
	// the last one of the queue must be the longest one.
	pCtx->SetMaxTagLen(TagQueue.back().length());
    ...
    for (int i = 0; i < num_vertical; i++) {
        pos_y = Y + i * hint_distance;
        for (int j = 0; j < num_horizontal; j++){
            pos_x = X + j * hint_distance;

            string tag = TagQueue.front();
            TagQueue.pop();

            KeyMouse::ElementInfo element_info;
            element_info.rect.left = pos_x;
            element_info.rect.right = pos_x;
            element_info.rect.top = pos_y;
            element_info.rect.bottom = pos_y;

            pTagMap->insert(std::pair<string, KeyMouse::ElementInfo>(tag, element_info));

            POINT point;
            point.x = pos_x;
            point.y = pos_y;

            DrawTag(*phMainWnd, hWnd, hDC, point, tag.c_str(), font);
        }
    }
	ReleaseDC(hWnd, hDC);
	pCtx->SetTagMap(pTagMap);
...
LRESULT CALLBACK TransparentWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_ERASEBKGND: {
		...
		if (pCtx) {
            ...
            if (pCtx->GetMode() == KeyMouse::Context::SELECT_MODE) {
                t_tagmap = std::thread(SetTagMapThread, hWnd, bEnableCache);
            } else {
                t_tagmap = std::thread(SetTagMapThreadForHint, hWnd, bEnableCache);
            }
			t_tagmap.detach();
		}
	}
...
void DrawTag(HWND hMainWnd, HWND hTransWnd, HDC hdc, POINT point, const TCHAR* psText, KeyMouse::Font font) {
	...
	GetTextExtentPoint(hdc, psText, int(_tcslen(psText)), &TextSize);

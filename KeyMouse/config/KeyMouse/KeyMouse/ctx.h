...
public:
	...
	static constexpr int SELECT_HKBINDING_NUM = 5;
	static constexpr int NORMAL_HKBINDING_NUM = 7;
	...
	static LRESULT fnHKProc_DoubleClickPrefix_(const WndEventArgs& Wea);
	...
	static LRESULT fnHKProc_SelectModeDouble_(const WndEventArgs& Wea);
	static LRESULT fnHKProc_FastSelectMode_(const WndEventArgs& Wea);
	static LRESULT fnHKProc_HintMode_(const WndEventArgs& Wea);
	static LRESULT fnHKProc_HintModeSingle_(const WndEventArgs& Wea);
	static LRESULT fnHKProc_HintModeDouble_(const WndEventArgs& Wea);
	static LRESULT fnHKProc_FastHintMode_(const WndEventArgs& Wea);

	static void EscSelectMode_(const WndEventArgs& Wea);
	static void SelectModeHandler_(const WndEventArgs& Wea, WORD VirtualKey);
...
struct Profile {
	...
	bool enable_cache;
	bool show_hint_only_current_monitor;
    int hint_distance;
};
...
public:
    ...
    void SetFastHintState(const bool flag);
    const bool &GetFastHintState() const;

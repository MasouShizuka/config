...
std::map<std::string, int> Config::command_id_map_{
	...
	{"singleClickPrefix", SINGLECLICKPREFIX},
	{"doubleClickPrefix", DOUBLECLICKPREFIX},
	{"selectModeSingle", SELECTMODESINGLE},
	{"selectModeDouble", SELECTMODEDOUBLE},
	{"fastSelectMode", FASTSELECTMODE},
	{"hintModeSingle", HINTMODESINGLE},
	{"hintModeDouble", HINTMODEDOUBLE},
	{"fastHintMode", FASTHINTMODE}
};
Config::Config() {
...
Config::Config(const std::wstring& json_name) {
	...
	default_json_ = json({
		...
		{"keybindings", {
			...
			{"hintModeSingle", "disabled"},
			{"hintModeDouble", "disabled"},
			{"fastHintMode", "disabled"}
		}}
		});
...
Profile Config::ExtractProfile() {
	...
	profile.show_hint_only_current_monitor = profile_json["showHintOnlyCurrentMonitor"].get<bool>();
    profile.hint_distance = profile_json["hintDistance"].get<int>();
...
Context::Context() :
	...
	if (profile_.invert_click_type) {
		click_type_ = SINGLE_RIGHT_CLICK;
	}
	else {
		click_type_ = SINGLE_LEFT_CLICK;
	}
}
...
void Context::SetFastHintState(const bool flag) {
	on_fast_hint_mode_ = flag;
}

const bool& Context::GetFastHintState() const {
	return on_fast_hint_mode_;
}

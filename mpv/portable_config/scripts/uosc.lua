...
function Menu:open(items, open_item, opts)
	...
	elements:add('menu', Element.new({
		...
		prev = function(this)
			if not this.selected_item then
                if this.active_item then
                    this.selected_item = this.active_item
                else
                    this.selected_item = this.scroll_height > this.scroll_step and this:get_centermost_visible_index() or this:get_last_visible_index()
                end
			end
			this.selected_item = math.max(this.selected_item - 1, 1)
			this:scroll_to_item(this.selected_item)
		end,
		next = function(this)
			if not this.selected_item then
			...
			this.selected_item = math.min(this.selected_item + 1, #this.items)
			this:scroll_to_item(this.selected_item)
		end,
		prev_page = function(this)
			this.selected_item = round(this.scroll_y / this.scroll_step) + 1
			this:scroll_to_item(this.selected_item)
		end,
		next_page = function(this)
			this.selected_item = round((this.scroll_y + this.height) / this.scroll_step)
			this:scroll_to_item(this.selected_item)
		end,
		first = function(this)
...
function Menu:enable_key_bindings()
	...
	if options.menu_hjkl_navigation then
		...
		menu:add_key_binding('shift+j', 'menu-next_page3',   self:create_action('next_page'), 'repeatable')
		menu:add_key_binding('k',       'menu-prev3',        self:create_action('prev'), 'repeatable')
		menu:add_key_binding('shift+k', 'menu-prev_page3',   self:create_action('prev_page'), 'repeatable')
		...
		menu:add_key_binding('g',       'menu-first',        self:create_action('first'))
		menu:add_key_binding('shift+g', 'menu-last',         self:create_action('last'))
	end
...
state.context_menu_items = (function()
	...
	for line in io.lines(input_conf_path) do
        ...
        -- remove extra symbol at end of line on windows
		local key, command, title = string.match(line, '%s*([%S]+)%s+(.*)%s#!%s*(.*).')
		if key then
...
mp.add_key_binding(nil, 'flash-top_bar', function()
...
mp.add_key_binding(nil, 'playlist', function()
	...
	function handle_playlist_change()
		if menu:is_open('playlist') then
            ...
            elements.menu:scroll_to_item(active_item)
		end
	end

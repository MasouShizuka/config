...
function Menu:enable_key_bindings()
    ...
    if options.menu_hjkl_navigation then
        ...
        -- modified
        menu:add_key_binding('d',       'menu-next_page3',   self:create_action('next_page'), 'repeatable')
        menu:add_key_binding('u',       'menu-prev_page3',   self:create_action('prev_page'), 'repeatable')
        menu:add_key_binding('g',       'menu-first3',       self:create_action('first'))
        menu:add_key_binding('shift+g', 'menu-last3',        self:create_action('last'))
    end
...
function create_multi_subtitles_menu_opener()
    return function()
        ...
        active_item = active_item and active_item + 1 or 1
        table.insert(items, 1, {hint = 'clear', value = 0})

        menu:open(items, function(id)
            multi_subtitles_toggle(id)

            menu:close()
        end, {type = 'sub', title = 'Multi-Subtitles', active_item = active_item})
    end
end

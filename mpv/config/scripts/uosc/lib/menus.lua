...
function create_self_updating_menu_opener(opts)
	return function()
		...
		if opts.on_move then
			actions[#actions + 1] = {
				name = 'move_up', icon = 'arrow_upward', label = ulang._playlist_submenu_label1 .. ' (Ctrl+UP/PGUP/HOME)'
			}
			actions[#actions + 1] = {
				name = 'move_down', icon = 'arrow_downward', label = ulang._playlist_submenu_label2 .. ' (Ctrl+DOWN/PGDWN/END)'
			}
		end
